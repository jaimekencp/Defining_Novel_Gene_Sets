function [R eff]=randmio_und_bipartite(R, ITER)
...
%   Jan 2021: Adjusted to fit on a (bipartite) nxm matrix (is the same as
%   directed version) (Martijn van den Heuvel). Run the function twice on
%   nxm and mxn for randomization
n=size(R,1);
[i j]=find(tril(R));
K=length(i);
ITER=K*ITER;
